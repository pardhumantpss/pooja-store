...
export function App() {
  ...
  // Periodic Stock Sync (every 6 seconds for real-time inventory tracking)
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0c] flex flex-col font-sans text-slate-300 selection:bg-indigo-500 selection:text-white">
      ...
      {/* 2. Interactive Category & Subcategory Pill Strip */}
      <CategoryNav
        categories={categories}
        selectedCategory={selectedCategory}
        onSelectCategory={(catId) => {
      ...
      {/* 3. Hero Feature Spotlight Banner (Curated & Functional) */}
        ...
        <div className="relative rounded-3xl overflow-hidden bg-[#161b22] text-white p-6 sm:p-10 border border-white/10 shadow-2xl">
          <div className="relative z-10 max-w-2xl space-y-3 sm:space-y-4">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 text-xs font-bold uppercase tracking-wider">
              ...
              <span>Autumn Editorial Drop • Real-Time Stock Sync</span>
            </div>

            <h1 className="text-2xl sm:text-4xl font-bold tracking-tight font-serif leading-tight text-white">
              Precision Crafted Hardware, Apparel & Audio.
            </h1>

            <p className="text-xs sm:text-sm text-slate-400 leading-relaxed">
              Every item is tracked at warehouse SKU resolution. Enjoy 256-bit encrypted checkout, complimentary express delivery over $100, and our intelligent AI Shopping Concierge.
            </p>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              ...
              <button
                id="hero-ai-concierge-btn"
                onClick={() => setIsAiOpen(true)}
                className="px-4 py-2.5 bg-white/5 hover:bg-white/10 text-white rounded-xl text-xs font-semibold transition flex items-center gap-1.5 cursor-pointer border border-white/10"
              >
                <Sparkles className="w-3.5 h-3.5 text-indigo-400" />
      ...
      {/* 4. Main Catalog Content Area (Sidebar + Grid) */}
      ...
      <footer className="bg-[#0d0d11] border-t border-white/10 mt-16 py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-4 gap-8 text-xs text-slate-400">
          <div className="space-y-3">
            <span className="font-bold text-base text-white tracking-wider block">AURA COMMERCE</span>
            <p className="text-slate-400 leading-relaxed">
              Curated hardware, technical textiles, and gourmet lifestyle essentials backed by real-time inventory and instant encrypted payment gateways.
            </p>
          </div>
          ...
          <div>
            <h4 className="font-bold text-white uppercase tracking-wider mb-3">Customer Care</h4>
            <ul className="space-y-2">
              <li>
              ...
              <li>
                <button onClick={() => setIsAiOpen(true)} className="hover:text-white transition cursor-pointer">
                  AI Shopping Concierge
                </button>
              </li>
              <li>
                <button onClick={() => setIsAdminOpen(true)} className="hover:text-amber-400 transition cursor-pointer">
                  Warehouse Stock Portal
                </button>
              </li>
              <li>
          ...
          <div>
            ...
            <p className="text-slate-400 leading-relaxed mb-3">
              Protected by PCI-DSS Level 1 compliant gateways with dynamic 3D-Secure 2.0 tokenization.
            </p>
